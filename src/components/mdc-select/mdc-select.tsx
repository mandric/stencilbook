...
export class MDCSelectElement {
  ...
  componentDidLoad() {
    const el = new MDCSelect(
        // support shadow root if in use
        this.el.shadowRoot ? 
        this.el.shadowRoot.querySelector('.mdc-select') :
        this.el.querySelector('.mdc-select')
    );

    el.listen('MDCSelect:change', () => {
      console.log(`Selected option at index ${el.selectedIndex} with value "${el.value}"`);
    });
  }

  render() {
    const { disabled, label, selectWidth } = this;
    const selectClasses = {
      'mdc-select': true,
      ...
      >
        {this.native ?
          ([
            ...
            <select class="mdc-select__native-control" disabled={disabled}>
              {this.items.map(item =>
